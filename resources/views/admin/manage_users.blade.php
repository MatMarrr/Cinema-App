...
@section('content')
    ...
    @if(session()->has('showDeleteUserModal') && session()->has('user_name'))
                    ...
                    <div>
                        <div class="mt-3 text-center sm:mt-5">
                            <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Are you sure that
                                you want to delete: {{session()->get('user_name')}}</h3>
                        </div>
                    </div>
                    <div class="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
                        <form action="{{ route('admin.delete.user', ['user_id' => session()->get('user_id')]) }}" method="POST" class="w-full sm:col-start-2">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
